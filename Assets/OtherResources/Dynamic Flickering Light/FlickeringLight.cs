...
public class FlickeringLight : MonoBehaviour {
    ...
    void Start ()
    {
        lightN = GetComponent<Light>();

        intensityOrigin = lightN.intensity;
        rangeOrigin = lightN.range;
        ...
        intensityOffset = lightN.intensity * scale;
        rangeOffset = lightN.range * scale;
    ...
    private void IntensityAndRange()
    {
        ...
        {
            intensityDelta = (intensityOrigin + Random.Range(-intensityOffset, intensityOffset) - lightN.intensity) * speed;


            rangeTarget = rangeOrigin + Random.Range(-rangeOffset, rangeOffset);
            rangeDelta = (rangeTarget - lightN.range) * speed;

            setNewTargets = false;
        }

        lightN.intensity += intensityDelta;
        lightN.range += rangeDelta;


        if (Mathf.Abs(lightN.range - rangeTarget) < 5f * scale) //////// CHECK IF TARGET IS NEAR ENOUGH. /////////
            setNewTargets = true;
